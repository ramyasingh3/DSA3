def coinChange(coins: list[int], amount: int) -> int:
    ...
    Returns:
        Minimum number of coins needed, or -1 if it's impossible
    """
...
if __name__ == "__main__":
    ...
    print(f"Output: {coinChange(coins1, amount1)}")  # Expected: 3 (5 + 5 + 1)
    
    # Test case 2
    coins2 = [2]
    amount2 = 3
    print(f"\nInput: coins = {coins2}, amount = {amount2}")
    print(f"Output: {coinChange(coins2, amount2)}")  # Expected: -1 (impossible)
    
    # Test case 3
    coins3 = [1]
    amount3 = 0
    print(f"\nInput: coins = {coins3}, amount = {amount3}")
    print(f"Output: {coinChange(coins3, amount3)}")  # Expected: 0 (no coins needed) 